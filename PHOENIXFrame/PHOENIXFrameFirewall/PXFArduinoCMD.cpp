...
void PXFArduino::OnCMD(String &cmdStr)
{
  ...
  if (mCMDIndexTemp > 0)
  {
    ...
    {
      long duration;
      if(millis() - mDistCheckLastTime > 23)
      {
        mDistCheckMeasureFlag = true; 
      }
      if (mDistCheckMeasureFlag)
      {
         _DistTest(); 
    ...
    else if (sOptTypeStr[OT_MP3_PLAY]==cmdCH)
    {
      _MP3Play();
    }
    ...
    else if (sOptTypeStr[OT_HX711_I]==cmdCH)
    {
      int index = _Str2Int(mCmdParams[1]);
    ...
    else if (sOptTypeStr[OT_HX711_TEST]==cmdCH)
    {
      int index = _Str2Int(mCmdParams[1]);
