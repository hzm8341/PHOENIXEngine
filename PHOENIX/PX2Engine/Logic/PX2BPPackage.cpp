...
void BPPackage::OnChildDetach(Movable *child)
{
	...
	if (logicGroup)
	{
		std::vector<Pointer0<BPPackage> >::iterator itGroup = mBPPackages.begin();
	...
	if (logicFile)
	{
		std::vector<Pointer0<BPFile> >::iterator itFile = mBPFiles.begin();
...
{
	std::vector<Pointer0<BPPackage> >::iterator it = mBPPackages.begin();
...
{
	std::vector<Pointer0<BPFile> >::iterator it = mBPFiles.begin();
...
void BPPackage::RegistProperties()
{
	...
	std::string tipText = mTipText->GetText()->GetText();
	AddProperty("Tip", Object::PT_STRING, tipText);
