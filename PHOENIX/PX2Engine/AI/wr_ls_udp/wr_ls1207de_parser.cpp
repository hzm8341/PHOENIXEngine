...
namespace wr_ls_udp
{
	...
	int CWrLs1207DEParser::Parse(char *data, size_t data_length, WrLsConfig &config, sensor_msgs::LaserScan &msg)
	{
		...
		{
			PX2_LOG_INFO("Invalid frame data!");
			return ExitSuccess;
		}
		...
		//PX2_LOG_USER("Publishing with frame id: %s", config.frame_id.c_str());
		PX2_LOG_USER("Publishing with frame id: %s", fFrame_id.c_str());
		...
		PX2_LOG_USER("scanning freq: %d, scan_time: %f", scanning_freq, msg.scan_time);
		...
		PX2_LOG_USER("starting_angle: %d, angle_min: %f", starting_angle, msg.angle_min);
		...
		PX2_LOG_USER("index_min: %d, angle_min: %f", index_min, msg.angle_min);
		...
		PX2_LOG_USER("index_max: %i, angle_max: %f", index_max, msg.angle_max);

		/*5: Fill data range*/
		msg.ranges.resize(index_max - index_min + 1);
		msg.ranges.assign(index_max - index_min + 1, std::numeric_limits<double>::infinity());
		PX2_LOG_USER("Fill sensor data. index_min = %d, index_max = %d.", index_min, index_max);
