...
int CWrLsCommon::StopScanner()
{
    ...
    if(0 != result)
    {
        // use printf because we couldn't use PX2_LOG_ERROR from destructor
...
bool CWrLsCommon::RebootDevice()
{
    ...
    if(0 != result)
    {
        PX2_LOG_ERROR("WR_LS - Error setting access mode");
    ...
    if(0 != result)
    {
        PX2_LOG_ERROR("WR_LS - Error rebooting device");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
    ...
    PX2_LOG_INFO("WR_LS - Rebooted scanner");
#endif
    return true;
}
...
CWrLsCommon::~CWrLsCommon()
{
    ...
    PX2_LOG_INFO("wr_ls_udp drvier exiting.\n");
}
...
int CWrLsCommon::InitScanner()
{
    ...
    if(0 != result)
    {
        PX2_LOG_ERROR("WR_LS - Error reading variable 'DeviceIdent'.");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
    ...
    if(0 != result)
    {
        PX2_LOG_ERROR("WR_LS - Error reading variable 'SerialNumber'.");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
    ...
    {
        PX2_LOG_ERROR("WR_LS - Error Unsuppored identify %s", strIdentify);
        return ExitFatal;
    }
    ...
    if(0 != result)
    {
        PX2_LOG_ERROR("WR_LS - Error reading variable 'FirmwareVersion'.");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
    ...
    if(0 != result)
    {
        PX2_LOG_ERROR("WR_LS - Error reading variable 'devicestate'.");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
    ...
    {
        PX2_LOG_USER("Laser scanner is ready.");
    }
    ...
    if(0 != result)
    {
        PX2_LOG_ERROR("WR_LS - Error when starting streaming 'LMDscandata'.");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
...
int CWrLsCommon::LoopOnce()
{
    ...
    if(0 != result)
    {
        PX2_LOG_ERROR("WR_LS - Read Error when getting datagram: %d", result);
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
    ...
    {
        PX2_LOG_USER("WR_LS - Received data gram. Data Length %d", dataLength);
        if(!mConnectFlag)
        {
            mConnectFlag = 1;
            PX2_LOG_INFO("WR_LS - Successfully connected !!");
        }

        if(memcmp(mRecvBuffer, header, sizeof(header)) == 0)                      //compare the header
        {
#if(0)
            PX2_LOG_INFO("%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-", *(mRecvBuffer + mDataLength + 0), *(mRecvBuffer + mDataLength + 1), *(mRecvBuffer + mDataLength + 2),   \
            ...
            {
                PX2_LOG_USER("frameTotalIndex:%d is out-of-order, last is:%d", frameTotalIndex, lastFrameTotalIndex);
            }
    ...
    {
        PX2_LOG_ERROR("WR_LS - Invalid data length!");
    ...
    if(start == NULL)
    {
        PX2_LOG_ERROR("WR_LS - Invalide data! Header NOT found!");
    }
...
{
    PX2_LOG_USER("Laser Message to send:");
    PX2_LOG_USER("Header  frame_id: %s", msg.header.frame_id.c_str());
    //PX2_LOG_USER("Header timestamp: %ld", msg.header.stamp);
    PX2_LOG_USER("angle_min: %f", msg.angle_min);
    PX2_LOG_USER("angle_max: %f", msg.angle_max);
    PX2_LOG_USER("angle_increment: %f", msg.angle_increment);
    PX2_LOG_USER("time_increment: %f", msg.time_increment);
    PX2_LOG_USER("scan_time: %f", msg.scan_time);
    PX2_LOG_USER("range_min: %f", msg.range_min);
    PX2_LOG_USER("range_max: %f", msg.range_max);
}
