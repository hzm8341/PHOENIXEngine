...
void showMap()
{
	...
	const AVector &moveDir = PX2_ROBOT.GetMoveDirection();
	...
	while (Slam2DManager::IsRun)
	{
		...
		float yRad1Adjust = (curY + PX2_ROBOT.mDegreeAdjust) * Mathf::DEG_TO_RAD;
		cvLine(LaserImage, cvPoint(robotX, robotY),
		...
		// first move degree
		float firstAxisY = PX2_ROBOT.mFirstMoveDirectionAxisY;
		...
		// move degree
		float moveDegree = PX2_ROBOT.GetMoveDirectionDegree() + PX2_ROBOT.mFirstMoveDirectionAxisY;
...
void Slam2DManager::Initlize()
{
	...
	int mapSize = hector_slam.getMapSize();
	float reso = hector_slam.getResolution();
	PX2_ROBOT.InitSlamMap(mapSize, reso);
}
//----------------------------------------------------------------------------
...
void Slam2DManager::Update(double appSeconds, double elapsedSeconds)
{
	if (Project::GetSingletonPtr())
	{
		...
		if (liDar)
		{
			...
			float offsetDegree = PX2_ROBOT.GetOffsetDegree();
			float yawCrossValue = PX2_ROBOT.GetYawCrossValue();
...
//#define PX2_SLAM2D_USEAXIS 1
					...
					for (int j = 0; j < (int)axisObjects.size() - 1; j++)
					{
							...
							lsd.TStamp <= axisObjects[j + 1].TStamp)
						{
							...
							if (Mathf::FAbs(axisObjects[j].AZ) < 15 && Mathf::FAbs(axisObjects[j + 1].AZ) < 15)
							{
								...
								PX2_ROBOT.SetCurMapUsingAxisWithOffset(curUseYSendToMap);
							}
			...
			if (1 == _ShowType)
			{
				...
				int mapSize = hector_slam.getMapSize();
				float reso = hector_slam.getResolution();
				PX2_ROBOT.SetSlam2DMap(map, mapSize, reso);
			}

			if (isHasAlreadyRuned)
			{
				int indexX = 0;
				int indexY = 0;
				float angle = 0;
				...
				float posX = ((float)indexX - mapSize*0.5f) * reso;
				float posY = ((float)indexY - mapSize*0.5f) * reso;
				PX2_ROBOT.SetSlam2DPosition(APoint(posX, posY, 0.0f), angle);
			}
