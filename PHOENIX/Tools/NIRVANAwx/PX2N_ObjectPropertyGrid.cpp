...
void ObjectPropertyGrid::OnPropertyGridChange(wxPropertyGridEvent &event)
{
	...
	for (int i = 0; i < (int)props.size(); i++)
	{
		...
		if (propObj.Tag == propName)
		{
			...
			if (propLabel == "Name")
			{
				Event *ent = EditorEventSpace::CreateEventX(EditorEventSpace::N_ObjectNameChanged);
			...
			if (em)
			{
				...
				else
				{
					...
					{
						Event *ent = EditorEventSpace::CreateEventX(EditorEventSpace::CurveChangedByPropertyGrid);
						ent->SetData<Object*>(obj);
						EventWorld::GetSingleton().BroadcastingLocalEvent(ent);
					}
				}
