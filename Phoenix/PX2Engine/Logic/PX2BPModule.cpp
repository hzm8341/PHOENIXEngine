...
void BPModule::RegistFunObj(const FunObject &funObj)
{
	...
	for (int i = 0; i < funObj.GetNumInParams(); i++)
	{
		...
		if (it == mInputParams.end())
		{
			...
			param->SetSize(fntMaxWidth0+12.0f, mItemHeight);
		}
	}
	...
	for (int i = 0; i < funObj.GetNumOutParams(); i++)
	{
		...
		if (it == mOutputParams.end())
		{
			...
			param->SetSize(fntMaxWidth1 + 12.0f, mItemHeight);
		}
	}

	mNameText->GetText()->SetText(funObj.Name);
	...
	bool isOriginalStart)
{
	if (MT_EVENT == mModuleType)
	{
		...
		for (int i = 0; i < (int)linkToParams.size(); i++)
		{
			...
			std::string str = std::string("PX2_EEH:AddEventCall(\"")
				+ mClassText->GetText()->GetText() + "\", \"" + GetName() + "\", "
				+ "\"" + linktoModule->GetFunctionName() + "\"" + ")\n";
			script += str;
		}
...
void BPModule::_ProcessOperations(std::string &script)
{
	...
	{
		//std::string scriptStr = PX2_LOGICM.GetGFScript(GetName());
	...
	{
		//std::string scriptStr = PX2_LOGICM.GetOPScript(GetName());
