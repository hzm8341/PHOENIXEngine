...
//---------------------------------------------------------------------------
int Serial::Open(const std::string &comport_in, int baudrate)
{
	int ret = 0;
	ret = Open(comport_in);
	if (0 != ret)
...
//---------------------------------------------------------------------------
std::string Serial::GetPortName(void)
{
	if (!mIsPortOpen) return "";

	return port_name;
}
//---------------------------------------------------------------------------
// Return the last error message in a (hopefully) user friendly format
std::string Serial::GetErroMessage(void)
{
	return error_msg;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
// set the baud rate
int Serial::SetBaudrate(int baud)
{
	if (baud <= 0) return -1;
	//printf("set_baud: %d\n", baud);
	baud_rate = baud;
	if (!mIsPortOpen) return -1;
#if defined(LINUX)
	speed_t spd;
	switch (baud) {
...
//---------------------------------------------------------------------------
int Serial::SetBaudrate(const std::string& baud_str)
{
	int baud = StringHelp::StringToInt(baud_str.c_str());
...
int Serial::Read(std::string &buffer)
{
	...
	if (r > 0)
		buffer = std::string((char*)buf, r);

	return r;
}
//---------------------------------------------------------------------------
int Serial::InputWait(int msec)
{
	if (!mIsPortOpen)
		return -1;
