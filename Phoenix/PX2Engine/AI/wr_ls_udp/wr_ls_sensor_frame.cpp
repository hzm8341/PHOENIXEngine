...
namespace wr_ls_udp
{
	...
	uint16_t CWrLsSensFrame::GetSensDataOfIndex(int index)
	{
		...
		{
			PX2_LOG_ERROR("Fail to get of index %d.", index);
			return 0;
		}
	...
	bool CWrLsSensFrame::InitFromSensBuff(char *buff, int length)
	{
		if (buff == NULL)
		{
			PX2_LOG_ERROR("Invalide input buffer!");
			return false;
		}
		...
		if (pData == NULL)
		{
			PX2_LOG_ERROR("Insufficiant memory!");
			return NULL;
		}
		...
		/*       if (true != CheckFrame((char*)m_pSensData->sens_data, dataCount * 2, m_pSensData->check_value))
			   {
			   PX2_LOG_ERROR("CheckFrame failed");
			   return false;
			   }*/
	...
	void CWrLsSensFrame::DumpFrameHeader()
	{
		...
		PX2_LOG_USER("Frame Header: 0x%02X", this->GetFrameHeader());
		PX2_LOG_USER("Command   ID: 0x%02X", this->GetCommandId());
		PX2_LOG_USER("Angle  START: 0x%04X", this->GetRangeStart());
		PX2_LOG_USER("Angle    END: 0x%04X", this->GetRangeEnd());
	}
	...
	void CWrLsSensFrame::DumpFrameData()
	{
		...
		int dataCount = this->GetSensDataCount();
		PX2_LOG_USER("Data   Count: %d", dataCount);
