...
int CWrLsCommonUdp::InitDevice()
{
    ...
    if (mSocket < 0)
    {
        PX2_LOG_ERROR("create udp socket failed");
        return ExitError;
    }
    ...
    if(h == NULL)
    {
        PX2_LOG_ERROR("unknown host '%s' /n", mHostName.c_str());
        return ExitError;
    }
    PX2_LOG_INFO("sending data to '%s' (IP : %s) (PORT : %s)", h->h_name, inet_ntoa(*(struct in_addr *)h->h_addr_list[0]), mPort.c_str());
    ...
    */
    ...
    {
        PX2_LOG_ERROR("bind socket failed");
        close(mSocket);
        mSocket = -1;
        return ExitError;
    }
...
int CWrLsCommonUdp::CloseDevice()
{
    if(mSocket != -1)
    {
        close(mSocket);
        mSocket = -1;
        PX2_LOG_ERROR("close socket and CloseDevice");
    }
}
...
int CWrLsCommonUdp::SendDeviceReq(const char *req, std::vector<unsigned char> *resp)
{
    if(mSocket == -1)
    {
        PX2_LOG_ERROR("SendDeviceReq: Socket NOT open");
    ...
    {
        PX2_LOG_ERROR("Write error for req command");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
...
int CWrLsCommonUdp::GetDataGram(unsigned char *receiveBuffer, int bufferSize, int *length)
{
    ...
    if(mSocket == -1)
    {
        PX2_LOG_ERROR("GetDataGram: Socket NOT open");
        mDiagUpdater.broadcast(diagnostic_msgs::DiagnosticStatus::ERROR,
