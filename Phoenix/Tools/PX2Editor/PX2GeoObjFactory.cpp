...
PX2::Node *GeoObjFactory::CreateTranslateCtrl_P()
{
	...
	vbaX.Position<Float3>(0) = Float3(startPos, 0.0f, 0.0f);
	...
	vbaY.Position<Float3>(0) = Float3(0.0f, startPos, 0.0f);
	...
	vbaZ.Position<Float3>(0) = Float3(0.0f, 0.0f, startPos);
...
PX2::Node *GeoObjFactory::CreateUICrossCtrl()
{
	...
	float centerSize = 24.0f;
	float centerSize1 = 48.0f;

	VertexBufferPtr vbCenter = new0 VertexBuffer(4, vf->GetStride());
	PolysegmentPtr segCenter = new0 Polysegment(vf, vbCenter, false);
	segCenter->SetNumSegments(2);
	VertexBufferAccessor vbaCenter(vf, vbCenter);
	vbaCenter.Position<Float3>(0) = Float3(0.0, 0.0f, centerSize);
	vbaCenter.Position<Float3>(1) = Float3(centerSize, 0.0f, 0.0f);
	vbaCenter.Position<Float3>(2) = Float3(0.0f, 0.0f, centerSize1);
	vbaCenter.Position<Float3>(3) = Float3(centerSize1, 0.0f, 0.0f);
	vbaCenter.Color<Float4>(0, 0) = Float4::BLACK;
	vbaCenter.Color<Float4>(0, 1) = Float4::BLACK;
	vbaCenter.Color<Float4>(0, 2) = Float4::BLACK;
	vbaCenter.Color<Float4>(0, 3) = Float4::BLACK;
...
//----------------------------------------------------------------------------
void GeoObjFactory::UpdateCameraCtrl(CameraNode *cameraNode, Polysegment *ps)
{
	...
	{
		float dMaxOverdMin = dMax / dMin;

		uuMin = uMin * dMaxOverdMin;
		rrMin = rMin * dMaxOverdMin;
	}
	else
	{
		uuMin = uMin;
	...
	VertexBufferAccessor vba(ps);

	// near
	vba.Position<Float3>(0) = Float3(rMin, dMin, uMin);
	vba.Position<Float3>(1) = Float3(rMax, dMin, uMin);

	vba.Position<Float3>(2) = Float3(rMin, dMin, uMax);
	vba.Position<Float3>(3) = Float3(rMax, dMin, uMax);

	vba.Position<Float3>(4) = Float3(rMin, dMin, uMin);
	vba.Position<Float3>(5) = Float3(rMin, dMin, uMax);

	vba.Position<Float3>(6) = Float3(rMax, dMin, uMin);
	vba.Position<Float3>(7) = Float3(rMax, dMin, uMax);

	// far
	vba.Position<Float3>(8) = Float3(rrMin, dMax, uuMin);
	vba.Position<Float3>(9) = Float3(rrMax, dMax, uuMin);

	vba.Position<Float3>(10) = Float3(rrMin, dMax, uuMax);
	vba.Position<Float3>(11) = Float3(rrMax, dMax, uuMax);

	vba.Position<Float3>(12) = Float3(rrMin, dMax, uuMin);
	vba.Position<Float3>(13) = Float3(rrMin, dMax, uuMax);

	vba.Position<Float3>(14) = Float3(rrMax, dMax, uuMin);
	vba.Position<Float3>(15) = Float3(rrMax, dMax, uuMax);

	// side
	vba.Position<Float3>(16) = Float3(rMin, dMin, uMin);
	vba.Position<Float3>(17) = Float3(rrMin, dMax, uuMin);

	vba.Position<Float3>(18) = Float3(rMin, dMin, uMax);
	vba.Position<Float3>(19) = Float3(rrMin, dMax, uuMax);

	vba.Position<Float3>(20) = Float3(rMax, dMin, uMin);
	vba.Position<Float3>(21) = Float3(rrMax, dMax, uuMin);

	vba.Position<Float3>(22) = Float3(rMax, dMin, uMax);
	vba.Position<Float3>(23) = Float3(rrMax, dMax, uuMax);

	for (int i = 0; i < 24; i++)
	{
		vba.Color<Float4>(0, i) = Float4::MakeColor(0, 0, 128, 255);
	}

	ps->UpdateModelSpace(Renderable::GU_MODEL_BOUND_ONLY);

	Renderer::UpdateAll(ps->GetVertexBuffer());
}
//----------------------------------------------------------------------------