...
void EU_PropertyGrid::OnEvent(Event *event)
{
	...
	else if (EditES::IsEqual(event, ProjectES::CloseProject))
	{
		_ClearSelect();
	}
}
...
void EU_PropertyGrid::_RefreshSelect()
{
	...
	if (1 == numSelObjs)
	{
		...
		for (int i = 0; i < (int)props.size(); i++)
		{
			...
			{
				UIButton *but = DynamicCast<UIButton>(item->GetObjectByName("Button"));
				if (but)
				{
		...
		if (name == propObj.Tag)
		{
			if ("LocalTransform" == propObj.Name || "WorldTransform" == propObj.Name)
			{
				...
				if (parent)
				{
					...
					{				
						val = APoint(val.X()*Mathf::DEG_TO_RAD,
							val.Y()*Mathf::DEG_TO_RAD,
							val.Z()*Mathf::DEG_TO_RAD);
