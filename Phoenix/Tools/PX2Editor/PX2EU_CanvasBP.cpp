...
PX2_IMPLEMENT_RTTI(PX2, Canvas, EU_CanvasBP);
...
//----------------------------------------------------------------------------
void EU_CanvasBP::OnLeftDown(const APoint &sceenPos, const APoint &logicPos)
{
...
//----------------------------------------------------------------------------
void EU_CanvasBP::OnLeftUp(const APoint &sceenPos, const APoint &logicPos)
{
	if (!PX2EU_MAN.IsSelectBP())
		return;
...
//----------------------------------------------------------------------------
void EU_CanvasBP::OnRightDown(const APoint &sceenPos, const APoint &logicPos)
{
	if (!PX2EU_MAN.IsSelectBP())
		return;
...
//----------------------------------------------------------------------------
void EU_CanvasBP::OnRightUp(const APoint &sceenPos, const APoint &logicPos)
{
	if (!PX2EU_MAN.IsSelectBP())
		return;
	...
	if (mIsRightPressed && !mIsMoved)
	{
		...
		float posSizePercentWdith = (logicPos[0] - canvasWorldRect.Left) / canvasWorldRect.Width();
		float posSizePercentHeight = (logicPos[2] - canvasWorldRect.Bottom) / canvasWorldRect.Height();
		...
		if (package)
		{
			CreateEditMenuOnBPPackage(logicPos);
		}
		else if (file)
		{
			APoint contentPos = logicPos;
			contentPos.X() += 2.0f;
			_ShowBPContentFrame(contentPos);
		}
		else if (module)
		{
			CreateEditMenuOnBPModule(logicPos);
		}
		else if (param)
		{
			if (param->GetNumLinkToParams() > 0 || param->GetNumLinkMeParams() > 0)
			{
				CreateEditMenuOnBPParam(logicPos);
			}
		}
	}
	...
	UIItem *parentItem)
{
	...
	const std::string &funName = funObject->Name;
	...
	for (int i = 0; i < (int)funObjectVec.size(); i++)
	{
		...
		UIItem *sItem = tree->AddItem(classItem, funObj->Name, funObj->Name);
		sItem->SetUserData("ClassName", className);
		sItem->SetUserData("Name", funObj->Name);
		sItem->SetUserData("Tag", std::string("ObjectFun"));
	}
}
//----------------------------------------------------------------------------
void EU_CanvasBP::_ShowBPContentFrame(const APoint &logicPos)
{
	if (!mBluePrintRenderWindow)
	{
		...
		mBluePrintRenderWindow = PX2EU_MAN.CreateUIWindow(
			mainRW, std::string("BluePrintTreeWindow"), std::string("BluePrint"), logicPos,
	...
	{
		mBluePrintRenderWindow->SetPosition(logicPos);
	...
	if (tree)
	{
		if (UICT_TREE_DOUBLE_SELECTED == type)
		{
			...
			if (selectItem)
			{
				...
				std::string funName;
				funName = selectItem->GetUserData<std::string>("Name", &funNameValied);
...
UIFrame *EU_CanvasBP::CreateBPContentFrame()
{
	...
	//sItemFS->SetUserData("Name", std::string("FunctionStart"));
	...
	//	sItem->SetUserData("ClassName", std::string(""));
	//	sItem->SetUserData("Name", optionStr);
	...
	//	sItem->SetUserData("ClassName", std::string(""));
	//	sItem->SetUserData("Name", operatorStr);
...
//----------------------------------------------------------------------------
void EU_CanvasBP::OnMotion(const APoint &sceenPos, const APoint &logicPos)
{
	if (!PX2EU_MAN.IsSelectBP())
		return;
...
//----------------------------------------------------------------------------
void EU_CanvasBP::OnMouseWheel(const APoint &sceenPos, const APoint &logicPos, float delta)
{
	if (!PX2EU_MAN.IsSelectBP())
		return;
	...
	if (!rectWorld.IsInsize(logicPos.X(), logicPos.Z()))
		return;
...
void EU_CanvasBP::CreateEditMenuOnBPModule(const APoint &pos)
{
	...
	PX2EU_MAN.Menu_Edit_AddItem("BP", "BPModule", "Delete", PX2_LM_EDITOR.V("Delete"), "n_DeleteSelection()");
...
void EU_CanvasBP::CreateEditMenuOnBPParam(const APoint &pos)
{
	...
	PX2EU_MAN.Menu_Edit_AddItem("BP", "BPParam", "Disconnect", PX2_LM_EDITOR.V("Disconnect"), "n_DisconnectParam()");
