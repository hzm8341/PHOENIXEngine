...
void EU_ProjectTree::OnEvent(Event *ent)
{
	...
	if (GraphicsES::IsEqual(ent, GraphicsES::AddObject))
	{
		...
		WeakPointer0<Object> parentObj = objData.ParentObj;
	...
	else if (EditorEventSpace::IsEqual(ent, EditorEventSpace::FindActorInProjectTree))
	{
		Object *obj = ent->GetData<Object*>();
		UIItem *item = GetItemByObject(obj);
		if (item)
...
//----------------------------------------------------------------------------
void EU_ProjectTree::OnSizeNodePicked(const CanvasInputData &inputData)
{
	UITree::OnSizeNodePicked(inputData);

	if (UIPT_RELEASED == inputData.PickType &&
		CanvasInputData::MouseTag::MT_RIGHT == inputData.TheMouseTag)
	{
