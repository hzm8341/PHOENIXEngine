...
EU_ResFrame::EU_ResFrame()
{
	...
	mTreeCanvas = new0 Canvas();
	mTreeCanvas->CreateUICameraNode();
	...
	mTreeCanvas->SetClearColor(blackGroundColor);
	...
	mSplitter->SetChildPickOnlyInSizeRange(false);
	...
	mRightFrame = new0 UIFrame();
	AttachChild(mRightFrame);
	mRightFrame->SetAnchorHor(0.0f, 1.0f);
	mRightFrame->SetAnchorParamHor(dirWidth + spSize, 0.0f);
	mRightFrame->SetAnchorVer(0.0f, 1.0f);
	mRightFrame->SetAnchorParamVer(0.0f, -PX2_UISM.Size_ToolBar);

	CanvasPtr canvas = new0 Canvas();
	canvas->CreateUICameraNode();
	blackGroundColor = Float4(PX2_UISM.Color_ViewBackground.X(),
		PX2_UISM.Color_ViewBackground.Y(), PX2_UISM.Color_ViewBackground.Z(), 1.0f);
	canvas->SetClearColor(blackGroundColor);
	mRightFrame->AttachChild(canvas);
	canvas->SetAnchorHor(0.0f, 1.0f);
	canvas->SetAnchorVer(0.0f, 1.0f);
	canvas->SetAnchorParamVer(resFramePickInfoHeight, 0.0f);

	mResGrid = new0 EU_ResGridFrame();
	canvas->AttachChild(mResGrid);
	mResGrid->SetAnchorHor(0.0f, 1.0f);
	mResGrid->SetAnchorVer(0.0f, 1.0f);

	mResList = new0 EU_ResList();
	canvas->AttachChild(mResList);
	mResList->SetAnchorHor(0.0f, 1.0f);
	mResList->SetAnchorVer(0.0f, 1.0f);
	mResList->Show(false);

	mResFramePickInfo = new0 UIFrame();
	mRightFrame->AttachChild(mResFramePickInfo);
...
void EU_ResFrame::_SetFramePos()
{
	...
	mRightFrame->SetAnchorHor(0.0f, 1.0f);
	mRightFrame->SetAnchorParamHor(leftWidth + mSplitter->GetWidth(), 0.0f);
}
//----------------------------------------------------------------------------
