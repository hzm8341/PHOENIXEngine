...
void TimeLineEdit::RemoveGroup(PX2::Object *obj)
{
	...
	for (; it != mCurveGroups.end(); it++)
	{
		if ((*it)->mObject == obj)
		{
			...
			UICurveGroup *uiGroup = _GetUICurveGroupFormCanvas(mCurveEditNode_UIGroup, group);
			if (uiGroup)
			{
...
void TimeLineEdit::ClearAllGroups()
{
	...
	for (int i = 0; i < (int)mCurveEditNode_UIGroup->GetNumChildren(); i++)
	{
		UICurveGroup *uiGroup = DynamicCast<UICurveGroup>(
			mCurveEditNode_UIGroup->GetChild(i));
...
//----------------------------------------------------------------------------
PX2::UICurveGroup *TimeLineEdit::_GetUICurveGroupFormCanvas(PX2::Node *frame, 
	CurveGroup *group)
{
	if (!frame) return 0;
