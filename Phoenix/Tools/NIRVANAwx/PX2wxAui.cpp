	...
	if (euMan)
	{
		...
		if (theme)
		{
			...
			if (0 == mType)
				...
				color = Float3TowxColour(Float3::MakeColor(10, 10, 10));
...
//----------------------------------------------------------------------------
int PX2wxAuiNotebook::DoModifySelection(size_t n, bool events)
{
	for (int i = 0; i < (int)GetPageCount(); i++)
	{
		wxWindow *wnd = GetPage(i);
		N_Frame *nFrame = wxDynamicCast(wnd, N_Frame);
		if (nFrame)
		{
			if (n == (size_t)i)
			{
				nFrame->SetTableSelect(true);
			}
			else
			{
				nFrame->SetTableSelect(false);
			}
		}
	}
	...
	int* x_extent)
{
	...
	if (theme)
	{
		if (page.active)
		{
			...
			{
				dc.SetTextForeground(Float3TowxColour(theme->Color_Aui_TabbarTextBot_Active));
			}
		}
		else
		{
	...
	wxRect* out_rect)
{
	...
	DrawButtons(dc, rect, bmp, wxColour(255, 255, 255), button_state);
	...
	int /*active_idx*/)
{
	...
	menuPopup.Append(1000 + count + msTabMenuID_PinPage, "PinPage");
