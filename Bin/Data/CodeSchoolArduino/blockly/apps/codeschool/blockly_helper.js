...
function uploadClick() {
    ...
    uploadCode("updateArduino", code, function(status, errorInfo) {
        if (status == 200) {
...
function resetClick() {
    ...
    uploadCode("resetArduino", code, function(status, errorInfo) {
        if (status != 200) {
...
function net_saveCode(filename)
{
	var code = Blockly.Arduino.workspaceToCode();
    
    uploadCode("saveArduino_"+filename, code, function(status, errorInfo) {
        if (status == 200) {
            alert("Program uploaded ok");
        } else {
            alert("Error uploading program: " + errorInfo);
