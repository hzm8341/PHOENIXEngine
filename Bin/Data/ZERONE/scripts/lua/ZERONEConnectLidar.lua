...
function zo_ConnectLidar()
    ...
    local btnLoadUpdateObs = UIButton:New("BtnUpdateObs")
    uiFrame:AttachChild(btnLoadUpdateObs)
    btnLoadUpdateObs.LocalTransform:SetTranslateY(-1.0)
    btnLoadUpdateObs:SetSize(150, 50)
    btnLoadUpdateObs:SetAnchorHor(0.5, 0.5)
    btnLoadUpdateObs:SetAnchorParamHor(-260.0, -260.0)
    btnLoadUpdateObs:SetAnchorVer(0.0, 0.0)
    btnLoadUpdateObs:SetAnchorParamVer(60.0, 60.0)
    btnLoadUpdateObs:SetSize(60, 60)
    btnLoadUpdateObs:SetStateColor(UIButtonBase.BS_HOVERED, Float3:MakeColor(255, 255, 255))
    btnLoadUpdateObs:SetStateColor(UIButtonBase.BS_NORMAL, Float3:MakeColor(255, 255, 255))
    btnLoadUpdateObs:SetStateColor(UIButtonBase.BS_PRESSED, Float3:MakeColor(190, 190, 190))
    btnLoadUpdateObs:GetPicBoxAtState(UIButtonBase.BS_NORMAL):SetTexture("Data/ZERONE/images/btnblue.png")
    btnLoadUpdateObs:GetPicBoxAtState(UIButtonBase.BS_NORMAL):SetPicBoxType(UIPicBox.PBT_NINE)
    local fText = btnLoadUpdateObs:CreateAddText(""..PX2_LM_APP:GetValue("UpdateObs"))
    zo_UISetTextFont(fText:GetText(), 16)
    btnLoadUpdateObs:SetScriptHandler("zo_ButFrameCallabck_Lidar")
...
function zo_ButFrameCallabck_Lidar(ptr, callType)
    ...
    elseif UICT_RELEASED==callType then
        ...
        elseif "BtnDlgRight"==name then
            ...
            if Application.PLT_WINDOWS==platType or Application.PLT_LINUX==platType then
                if nil~=PX2_ROBOT:GetLidar() then
                    if PX2_ROBOT:GetLidar():IsOpened() then
                        PX2_ROBOT:GetLidar():Close()
                    else
        ...
        elseif "BtnSaveMap"==name then
            PX2_ROBOT:SaveMap("map.px2obj")
        elseif "BtnLoadMap"==name then
            PX2_ROBOT:LoadMap("map.px2obj", APoint(0.01, 0.01, 0.0), 0.001)
        elseif "BtnUpdateObs"==name then
             PX2_ROBOT:UpdateSceneObstacles()
        end
    elseif UICT_CHECKED == callType then
        if "SlamCheckButton"==name then
            PX2_ROBOT:SetSlamMapUpdate(true)
        end
