...
function ZERONEUIFaceCtrl:OnAttached()
    ...
    textPosition:SetAnchorParamVer(-60.0, -60.0)
    textPosition:SetSize(500.0, 100.0)
    textPosition:GetText():SetAutoWarp(true)
    textPosition:SetPivot(0.5, 0.5)
    ...
    textPositionCur:SetAnchorParamVer(-80.0, -80.0)
    textPositionCur:SetSize(500.0, 100.0)
    textPositionCur:GetText():SetAutoWarp(true)
    textPositionCur:SetPivot(0.5, 0.5)
...
function ZERONEUIFaceCtrl:OnPUpdate()
    local pos = PX2_ROBOT:GetPosition()
    ...
    local txt = "P:"..StringHelp:FloatToString(pos:X())..","..StringHelp:FloatToString(pos:Y())..","..StringHelp:FloatToString(pos:Z()).." D:"..string.format("%.2f", degree)
    self._textPosition:GetText():SetText(txt)

    local txtCur = "C_P:"..string.format("%.2f", zo_MapPickPos.x) ..","..string.format("%.2f", zo_MapPickPos.y)
...
function ZERONEUIFaceCtrl:CreateLidarFrame()
    ...
    local picBox = PX2_ROBOT:GetUIFPicBoxMap()
    lidarFrame:AttachChild(picBox)
    picBox.LocalTransform:SetTranslateY(-5.0)
    picBox:SetAnchorHor(0.5, 0.5)
    picBox:SetAnchorVer(0.5, 0.5)
    picBox:SetAlpha(0.5)
    picBox:SetSize(640.0, 640.0)
    picBox:SetScriptHandlerNodePicked("ZERO_MapPicBoxCallback")
    ...
    local picBoxCur = PX2_ROBOT:GetLidar():GetUIFPicBoxLidar()
...
function ZERO_SetLogicPos(obj, logicPos)
    ...
    local mapSize = PX2_ROBOT:GetMapSize()
    local mapReso = PX2_ROBOT:GetResolution()
...
function ZERO_SetObst(obj, logicPos)
    ...
    if 0 == ZERONE_MAP_DrawType then -- clear
        PX2_ROBOT:SetValueAtPos(pos, 0.25, 200)
    
    elseif 1 == ZERONE_MAP_DrawType then -- obst
        PX2_ROBOT:SetValueAtPos(pos, 0.1, 0) -- 

    elseif 2 == ZERONE_MAP_DrawType then

        PX2_ROBOT:SetValueAtPos(pos, 0.1, 201) -- space can go
    end
end
...
function ZERONEUIFaceCtrl:CreateBtnsSetting()
    ...
    comBoxEdit:AddChooseStr("Clear")
    comBoxEdit:AddChooseStr("AddObst")
    comBoxEdit:AddChooseStr("Remove")
    comBoxEdit:SetChooseListHeightSameWithChooses()
    comBoxEdit:SetAnchorHor(1.0, 1.0)
    comBoxEdit:SetAnchorVer(0.0, 0.0)
    comBoxEdit:SetAnchorParamHor(-90.0, -90.0)
    comBoxEdit:SetAnchorParamVer(160.0, 160.0)
    comBoxEdit:SetSize(120.0, 30.0)
